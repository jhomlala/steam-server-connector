...
public class SourceRcon {
    ...
    public static String send(String ipStr, int port, String password, String command, int localPort) throws BadRcon, ResponseEmpty, IOException {
        ...
        if (InetAddress.getByName(ipStr).isReachable(2000)==true)  
        	{  
        	...
        	System.out.println("Checking connection:" + network);
        	if (network)
        	{
            ...
            if (rcon_auth(password)) {
                ...
                if (resp != null) {
                    ...
                    if (response.length() == 0) {
                        //Controller.ConsoleError("CANT CONNECT TO SERVER!");
                    }
                }
            }
            else {
            	Controller.ConsoleError("BAD RCON");
            }

        return response;
        	}
        	else
        	Controller.ConsoleError("CANT CONNECT TO THIS SERVER");
			return response;
    }
